...
def processaURL(urlprocessar, customaadd):
    ...
    querySelect = (
        "SELECT * FROM URLManager WHERE URL = '{0}'".format(urlprocessar)
    )

    cursor.execute(querySelect)
    if len(cursor.fetchall()) < 1:
        query = (
            "SELECT case HASH when HASH IS NULL THEN '0' else '1'"
            "end from URLManager where HASH = '{0}'".format(urlFinal)
        )
        ...
        if sqlquery == "None":
            query2 = (
                "INSERT INTO URLManager(HASH, URL, DATE)"
                "VALUES('{0}','{1}','{2}')".format(
                    urlFinal,
                    urlprocessar,
            ...
            CompleteURL = "http://{0}{1}u/{2}".format(
                BSUrl,
                BSFolder,
                urlFinal
            )
        ...
        querySelectHASH = (
            "SELECT HASH FROM URLManager WHERE URL = '{0}'".format(
                urlprocessar)
        )
...
def CheckURL(urlcode):
    ...
    cursor = connection.cursor()
    CheckQuery = (
        "SELECT URL from URLManager where HASH = '{0}'".format(urlcode))
